...
if (TRUE){
  option_list <- list(
    ...
    make_option(c("-t", "--CI"), type="double", default=0.95,
                help="置信区间"),
    make_option(c("-n", "--Min"), type="double", default=5,
                help="组中最大值，最小的那个被分成多少份")
    )
  opts <- parse_args(OptionParser(option_list=option_list))
}

Minseq <- function(x){# x <- 3155.8
  nn <- nchar(x)
  nn1 <- round(x * 10^-(nn-1),digits = 0)
  nn2 <- round(x * 10^-(nn-1),digits = 1)
  ifelse(nn2 < (nn1+0.5),nn1,nn1+0.5) * 10^(nn-1)
}
...
data1 <- read_tsv(opts$input) %>% rename("OTUID" = colnames(.)[1])
...
if (opts$map != "none"){
  ...
  DD <- data1 %>% .[,-1] %>% t %>% 
    ...
    group_by(group) %>% nest %>% 
    mutate(data = map(data,function(x) x %>% rename_all(~data1$OTUID)))
  
  DD1 <- DD %>% mutate(data = map(data,function(x) 
    Specaccum(comm = x,permutations = opts$per)[[1]])) %>% unnest(data)
  DD2 <- DD %>% mutate(data = map(data,function(x) 
    Specaccum(comm = x,permutations = opts$per)[[2]])) %>% unnest(data)

  DD4 <- DD1 %>% group_by(group) %>% nest %>% mutate(data = map_dbl(data,function(x) nrow(x)))
  
  bY1 <- ifelse(max(DD4$data) >= opts$Min & max(DD4$data) >= 2*opts$Min,
               opts$Min,max(DD4$data))

  bY2 <- seq(from=1,to=max(DD4$data),length.out = bY1) %>% 
    map_dbl(.,function(x) round(x,digits = 0))
  
  DD3 <- DD1 %>% filter(sites %in% bY2)
  ...
  p1 <- ggplot() + 
    ...
    geom_line(DD1,mapping = aes(x = sites, y = richness, 
                                group = group, color = group),size=0.6) +
  ...
  p2 <- ggplot() + 
    ...
    geom_point(DD3,mapping = aes(x = sites, y = richness, 
                                 group = group, color = group, shape = group),size=1) +
    scale_shape_manual(values=mypch,name="Group") +
    scale_colour_manual(values=as.vector(sc$V2),name="Group") +
    scale_fill_manual(values=as.vector(sc$V2),name="Group") +
    geom_errorbar(DD3,mapping = aes(x = sites, ymin=richness-ci, ymax=richness+ci, 
                                    color = group), width=.2, size = 0.1,show.legend = FALSE) +
    xlab("Number of Samples")+ylab(str_c("Number of OTUs(Mean ","\U00B1"," 95%CI",")")) +
    mytheme1() +
    theme(legend.position=c(1,0), legend.justification=c(1,0))
  ggsave("specaccum.groups.errorbar.pdf", p2, width = 6, height = 5)
