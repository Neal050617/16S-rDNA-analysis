...
if (TRUE){
  option_list <- list(
    ...
    make_option(c("-t", "--cvtime"), type="numeric", default=5,
                help="重复几遍n倍交叉验证"),
    make_option(c("-p", "--per"), type="numeric", default=0.001,
                help="OTU丰度筛选"),
...
if (f != "none"){
  ...
  data4 <- data4[data4$Mean_decrease_in_accuracy >= 0.001,]
...
## 训练集
# 寻找最优
result <- replicate(cvt, my.rfcv(Data[,-ncol(Data)], Data[,"group"],cv.fold = cvn,step=0.5), simplify=FALSE)
...
# 预测建模
train.rf=randomForest(Data[,marker.p],Data[,"group"],ntree=1000,proximity=TRUE,importance=TRUE)
...
pdf(pdf.dir2)
if (c != "none"){
  boxplot(train.p[, 2] ~ Data[,"group"], col = mycol, main = "Probability", names = levels(Data[,"group"]))
}else{
  boxplot(train.p[, 2] ~ Data[,"group"], col = 3:2, main = "Probability", names = levels(Data[,"group"])) 
}
dev.off() 
