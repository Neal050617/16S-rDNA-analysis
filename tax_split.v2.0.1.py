...
# taxonomy 分割 
def split_tax(datas,m):# datas,m = copy.deepcopy(data),""
...
def fix_tax(q):# q = copy.deepcopy(table)
    ...
    for i in range(1,Df4_2.shape[1]-2):# i = 4
        for k in range(Df4_2.shape[0]):# k = 278
            ...
            if bool(re.search(Df4_2.iloc[k,i]+"$",Df4_2.iloc[k,i-1],re.I)):
...
if __name__=='__main__':
    ...
    # 是否要对物种信息进行校正
    if opts.f == 'T':
        ttable = np.array(fix_tax(table)).tolist()
        ttable.insert(0,table[0])
        table = copy.deepcopy(ttable)
